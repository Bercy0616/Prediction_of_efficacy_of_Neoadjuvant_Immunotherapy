...
class ShuffleNetV2(nn.Module):
    def __init__(self, stages_repeats, stages_out_channels, num_classes=1000):
        ...
        self.classify = nn.Sequential(nn.Linear(output_channels, 128),
                                 nn.BatchNorm1d(128),
                                 nn.ReLU(inplace=True),
                                 nn.Linear(128, num_classes))
        
        self.classify2 = nn.Sequential(nn.Linear(output_channels, 512),
                                 nn.BatchNorm1d(512),
                                 nn.ReLU(inplace=True),
...
if __name__ == "__main__":##测试一下shuffenetv23d的输入输出
    net = shufflenet_v2_x0_5(num_classes=2)
    x = torch.randn(2, 1, 73, 60, 56)
    y,f = net(x)
    print(y.shape)
    print(f.shape)
